...
describe('Create Rental', () => {
  const dayAdd24Hours = (0, _dayjs.default)().add(1, 'days').toDate();
  beforeEach(() => {
  ...
  it('should not be able to create a new rental for a user in an open rental', async () => {
  ...
  it('should not be able to create a new rental for a car in an open rental', async () => {
    await rentalsRepositoryInMemory.create({
      car_id: 'test',
      expected_return_date: dayAdd24Hours,
      user_id: '12345'
  ...
  it('should not be able to create a new rental with return date below 24h', async () => {
    await expect(createRentalUseCase.execute({
      user_id: '123',
      car_id: 'test',
